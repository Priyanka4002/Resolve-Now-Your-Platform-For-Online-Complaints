...
/*app.post("/assignedComplaints", async (req, res) => {
  try {
    ...
    if (!complaint || complaint.status === "Cancelled") {
      return res.status(400).json({ error: "Cannot assign a cancelled complaint" });
    }
...
/*app.put("/complaints/:id/cancel", async (req, res) => {
  try {
    const { id } = req.params;

    // 1. Update the complaint status to "Cancelled"
    ...
    // 2. Remove the complaint from AssignedComplaint (if already assigned)
    await AssignedComplaint.deleteOne({ complaintId: id });

    res.json({ success: true, message: "Complaint cancelled and removed from agent", complaint: updated });
  } catch (err) {
...
app.put("/complaints/:id/cancel", async (req, res) => {
  try {
    ...
    // Update complaint status in ComplaintSchema
    const updatedComplaint = await ComplaintSchema.findByIdAndUpdate(
      id,
      { status: "Cancelled" },
      { new: true }
    ...
    // Update status in AssignedComplaint (if exists)
    ...
    res.json({
      success: true,
      message: "Complaint marked as Cancelled",
