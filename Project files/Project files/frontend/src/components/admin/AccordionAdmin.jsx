...
const AccordionAdmin = () => {
  ...
  const handleSelection = async (agentId, complaintId, status, agentName) => {
    try {
      ...
      // Update the complaint status locally to hide the button
      /*const updatedList = complaintList.map((complaint) =>
        complaint._id === complaintId ? { ...complaint, status: "Assigned" } : complaint
      );
                      ...
                      */}
                      {complaint.status === "completed" || complaint.status === "Cancelled" ? (
                        <div style={{ marginTop: "10px", color: "red", fontWeight: "bold" }}>
                          {complaint.status === "Cancelled" && "Complaint Cancelled by User"}
                        </div>
                      ) : (
                        <Dropdown className='mt-2'>
                          <Dropdown.Toggle variant="warning" id="dropdown-basic">
